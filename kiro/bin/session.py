...
body={"session_id":d.get("session_id",""),"cwd":cwd,"os_user":os.environ.get("USER",""),"model":d.get("model",""),"os_type":platform.system(),"hostname":platform.node(),"surface":"kiro_cli","kiro_identity":ki,"git_email":G(["git","config","user.email"]),"git_name":G(["git","config","user.name"]),"git_branch":br,"git_remote_url":ru,"jira_ticket_id":(mm.group(1) if mm else ""),"connection_type":("ssh" if os.environ.get("SSH_CONNECTION") else "local")};
